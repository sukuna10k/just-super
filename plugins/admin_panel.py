...
async def tutorial(bot: Client, message: Message):
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("• ᴘʀᴏᴘʀɪᴇᴛᴀɪʀᴇ", url="https://t.me/hyoshassistantbot"),
             InlineKeyboardButton("• ᴛᴜᴛᴏʀɪᴀʟ", url="https://t.me/hyoshassistantbot")]
        ])
    )
...
async def ban(c: Client, m: Message):
    if len(m.command) < 4:
        await m.reply_text(
            f"Utilisez cette commande pour bannir un utilisateur du bot.\n\n"
            f"Usage :\n\n"
            f"`/ban user_id ban_duration ban_reason`\n\n"
            f"Exemple : `/ban 1234567 28 Vous m'avez mal utilisé.`\n"
            f"Cela bannira l'utilisateur avec l'ID `1234567` pendant `28` jours pour la raison `Vous m'avez mal utilisé`.",
            quote=True
        )
